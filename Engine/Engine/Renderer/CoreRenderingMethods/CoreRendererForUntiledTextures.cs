        ...
        where I: IMemoryAlignment
    {
        ...
        if (min_b <= max_t)
        {
            for (uint y = min_t; y < max_b; y++, screenIndexPtr += width)
            {
                ...
                Vector256<uint> gathered = Vector256.GatherMask(
                    yV,
                    textureBuffer,
                    texelIndexV.AsInt32(),
                    maskV.As<uint, int>()
        ...
        if (min_b > max_t)
        {
            ...
            while (screenIndexPtr < screenIndexPtrEnd)
            {
                ...
                Vector256<uint> gathered = Vector256.Gather(
                    textureBuffer,
                    texelIndexV.AsInt32()
                );
        ...
        where I : IMemoryAlignment
    {
        ...
        if (min_b <= max_t)
        {
            for (uint y = min_t; y < max_b; y++, screenIndexPtr += width)
            {
                ...
                Vector128<uint> gathered = Vector128.GatherMask(
                    in yV,
                    textureBuffer,
                    texelIndexV.AsInt32(),
                    maskV.As<uint, int>()
        ...
        if (min_b > max_t)
        {
            ...
            while (screenIndexPtr < screenIndexPtrEnd)
            {
                ...
                Vector128<uint> gathered = Vector128.Gather(
                    textureBuffer,
                    texelIndexV.AsInt32()
                );
