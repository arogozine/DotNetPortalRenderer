...
internal static class WadReader
{
    ...
    private static Map ExtractDoomMap(WadFile wad, string mapName)
    {
        ...
        for (int i = 0; i < sectorDefs.Length; i++)
        {
            ...
            MapSector mapSector = new()
            {
                ...
                {
                    Texture = TextureCache.GetTexture(sector.FloorTexture),
                    Alpha = 0f,
                ...
                {
                    Texture = TextureCache.GetTexture(sector.CeilingTexture),
                    Alpha = 0f,
                    RenderingOptions = TextureRenderingOptions.None,
                    XOffset = 0,
                    XScale = null,
                ...
                CeilingShade = ClampSectorShade(sector.LightLevel)
            };
        ...
        foreach (MapSector sector in sectors)
        {
            foreach (Line line in sector.Walls)
            {
                ...
                if (line.SectorTo is int sectorTo)
                {
                    ...
                    {
                        GameTexture middleTexture = TextureCache.GetTexture(middleTextureInfo);
    ...
    private static int DetermineXOffset(GameTextureInfo textureInfo, GameTexture wallTexture)
    {
    ...
    {
        var defaultTexture = new GameTextureInfo
        {
        ...
        foreach (MapSector sector in sectors)
        {
            ...
            foreach (Line wall in sector.Walls)
            {
                if (wall.SectorTo is int sectorId)
                {
                    ...
                    {
                        wall.UpperTexture = new GameTextureInfo
                        {
                ...
                {
                    wall.MiddleTexture ??= new GameTextureInfo
                    {
                        Texture = TextureCache.GetTexture(skyTexture),
                        RenderingOptions = TextureRenderingOptions.Skybox,
                        Alpha = 0f,
    ...
    private static List<Sprite> ExtractSprites(ReadOnlySpan<UdmfThing> things)
    {
        ...
        Precalculations.PrecalculateWallSprites(CollectionsMarshal.AsSpan(sprites));
    ...
    private static Dictionary<ThingType, GameSpriteAnimation> GetMultiAngleSprites()
    {
    ...
    private static Map ExtractDoomMap(WadLump textLump, string mapName)
    {
        ...
        for (int i = 0; i < sectorDefs.Length; i++)
        {
            ...
            foreach (LineInfo lineInfo in lines)
            {
                ...
                GameTextureInfo? middleTexture = ToTextureInfo(lineInfo.MiddleTexture, lineInfo.XOffsetMid, lineInfo.YOffsetMid, lineInfo.LowerUnpegged);
        ...
        static GameTextureInfo GetCeilingTextureInfo(UdmfSector sector)
        {
        ...
        static GameTextureInfo GetFloorTextureInfo(UdmfSector sector)
        {
            return new GameTextureInfo
            {
    ...
    {
        return new LineVector(id, new Vector2(vertex.X, vertex.Y));
    }
    ...
    private static GameTextureInfo? ToTextureInfo(string? name, int xOffset, int yOffset, bool renderFromBottom)
    {
        if (name is null)
        {
            return null;
    ...
    private static short ClampSectorShade(short shade)
    {
        return (byte)(shade == 256 ? byte.MaxValue : shade);
    }
}
